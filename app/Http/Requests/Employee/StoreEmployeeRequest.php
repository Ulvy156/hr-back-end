    ...
    public function rules(): array
    {
        return [
            ...
            'employee_code' => [
                ...
                'string',
                'max:30',
                Rule::unique('employees', 'employee_code')->where(fn ($query) => $query->whereNull('deleted_at')),
            ],
            'department_id' => [
                'required',
                'integer',
            ...
            'email' => ['required', 'string', 'email', 'max:255', Rule::unique('employees', 'email')->where(fn ($query) => $query->whereNull('deleted_at'))],
            'phone' => ['required', ...$this->phoneFormatRules()],
            'date_of_birth' => ['nullable', 'date'],
            'gender' => ['nullable', Rule::enum(EmployeeGender::class)],
            'personal_phone' => ['nullable', ...$this->phoneFormatRules()],
            ...
            'emergency_contact_phone' => ['nullable', ...$this->phoneFormatRules()],
            ...
            'last_working_date' => ['nullable', 'date', 'after_or_equal:hire_date'],
