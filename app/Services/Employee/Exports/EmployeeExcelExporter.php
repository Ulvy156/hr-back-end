...
namespace App\Services\Employee\Exports;
...
class EmployeeExcelExporter
{
    ...
    public function store(string $path, array $report): void
    {
        ...
        if ($status !== true) {
            throw new \RuntimeException('Unable to create employee Excel export archive.');
        }
    ...
    private function rows(array $report): array
    {
        ...
        foreach ($report['records'] as $record) {
            $rows[] = [
                ...
                ['value' => $record['full_name'] ?? null, 'style' => 0],
                ['value' => $record['email'] ?? null, 'style' => 0],
                ['value' => $record['phone'] ?? null, 'style' => 0],
                ['value' => $record['department'] ?? null, 'style' => 0],
                ['value' => $record['current_position'] ?? null, 'style' => 0],
                ['value' => $record['manager'] ?? null, 'style' => 0],
                ['value' => $record['hire_date'] ?? null, 'style' => 0],
                ['value' => $record['employment_type'] ?? null, 'style' => 0],
